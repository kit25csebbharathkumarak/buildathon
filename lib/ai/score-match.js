...
export function scoreMatch({
  ...
  weights = { explicit: 0.4, transferable: 0.3, recency: 0.2, velocity: 0.1 },
}) {
  let explicitVectors = [];
  ...
  const wExp = Number(weights.explicit ?? 0.4);
  const wTrans = Number(weights.transferable ?? 0.3);
  const wRec = Number(weights.recency ?? 0.2);
  const wVel = Number(weights.velocity ?? weights.learning ?? 0.1);
  const wSum = wExp + wTrans + wRec + wVel || 1.0;

  const weightedExplicit = (wExp / wSum) * normExplicit;
  const weightedTransferable = (wTrans / wSum) * normTransferable;
  const weightedRecency = (wRec / wSum) * normRecency;
  const weightedVelocity = (wVel / wSum) * normVelocity;
